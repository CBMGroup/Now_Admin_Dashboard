    ...
    columnHelper.display({
      ...
      cell: (info) => (
        <button
          ...
          className="w-8 h-8 rounded-full bg-[#00D1C1] flex items-center justify-center hover:bg-[#00B8A9] transition-colors shadow-lg shadow-teal-500/20"
        >
    ...
    columnHelper.accessor('title', {
      header: ({ column }) => (
        <button
          ...
          className="flex items-center gap-2 hover:text-[#00D1C1] transition-colors"
        >
          Title
      ...
      cell: (info) => (
        <div className="flex flex-col">
          ...
          {info.row.original.is_explicit && (
            <span className="text-[8px] font-bold text-[#EF4444] border border-[#EF4444] w-fit px-1 rounded mt-0.5">E</span>
          )}
        </div>
      ),
    }),
    columnHelper.accessor('artist', {
      header: category === 'Podcast' ? 'Host' : category === 'Audiobooks' ? 'Author' : category === 'Poems' ? 'Poet' : 'Artist',
      cell: (info) => (
        <span className="text-[#A3A3A3] hover:text-[#00D1C1] cursor-pointer transition-colors">
          {info.getValue()}
        </span>
      ),
    }),
  ...
  if (error) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center text-red-500 space-y-4">
        ...
        <p className="text-xl font-bold">Error loading {title.toLowerCase()}</p>
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[#F1F1F1]">{title}</h1>
      ...
      <div className="bg-[#1A1A1A] rounded-xl border border-[#2A2A2A] p-4">
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            className="flex items-center gap-2 text-[#F1F1F1] hover:text-[#00D1C1] transition-colors"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#F1F1F1] mb-2">
                {category === 'Podcast' ? 'Host' : category === 'Audiobooks' ? 'Author' : category === 'Poems' ? 'Poet' : 'Artist'}
              </label>
              <select
                value={selectedArtist}
                onChange={(e) => setSelectedArtist(e.target.value)}
                className="w-full px-4 py-2 bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg text-[#F1F1F1] focus:outline-none focus:ring-2 focus:ring-[#00D1C1] focus:border-transparent"
              >
